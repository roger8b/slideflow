...
def build_cover(slide: dict) -> str:
    ...
    tid = nb.title("ROOT", c.get("title", "Untitled"),
                   fontSize=58, color=tc, textAlign="center", fontWeight="bold")
    children.append(tid)

    bar_id = nb.container("ROOT", children=[],
                          background=accent, height="2px", width="72%",
                          borderRadius=2, alignSelf="center", margin="18px 0 8px 0")
    children.append(bar_id)

    if c.get("subtitle"):
        sid = nb.text("ROOT", c["subtitle"],
                      fontSize=18, color=tc, textAlign="center", fontWeight="400")
        children.append(sid)

    return nb.build(children, {
        "background": bg, "alignItems": "center",
        "justifyContent": "center", "gap": 12, "padding": "56px 72px",
    })
...
def build_agenda(slide: dict) -> str:
    ...
    accent = slide.get("accent", "#e94560")
    tc = slide.get("text_color", "#ffffff")
    tc_muted = slide.get("text_color_muted", tc)

    nb = NodeBuilder()
    children = []

    header_row = nb.container("ROOT", children=[],
                              flexDirection="row", alignItems="center",
                              justifyContent="flex-start", gap=14,
                              width="100%", height="auto", padding=0)
    tid = nb.title(header_row, c.get("title", "Agenda"),
                   fontSize=64, color=tc, textAlign="left", fontWeight="bold")
    mark = nb.container(header_row, children=[],
                        width="56px", height="56px", padding=0,
                        display="flex", flexDirection="column",
                        alignItems="flex-start", justifyContent="flex-end",
                        gap=0, margin="8px 0 0 -6px", background="transparent")
    diag = nb.container(mark, children=[],
                        background=accent, width="10px", height="46px",
                        transform="rotate(-45deg)", margin="0 0 -8px 16px")
    base = nb.container(mark, children=[],
                        background=accent, width="48px", height="8px")
    stem = nb.container(mark, children=[],
                        background=accent, width="8px", height="30px",
                        alignSelf="flex-end", margin="-30px 0 0 0")
    nb.add_children(mark, [diag, base, stem])
    nb.add_children(header_row, [tid, mark])
    children.append(header_row)

    grid = nb.container("ROOT", children=[],
                        display="grid", gridTemplateColumns="1fr 1fr",
                        gap="30px 88px", width="100%", height="auto",
                        padding="44px 0 0 0", alignItems="stretch")

    start_number = c.get("start_number", 1)
    items = c.get("items", [])
    midpoint = (len(items) + 1) // 2
    columns = [items[:midpoint], items[midpoint:]]

    for col_idx, col_items in enumerate(columns):
        col = nb.container(grid, children=[],
                           flexDirection="column", gap=34, width="100%",
                           height="auto", alignItems="flex-start", padding=0)
        for row_idx, item in enumerate(col_items):
            n = start_number + row_idx + (0 if col_idx == 0 else midpoint)
            row_id = nb.container(col, children=[],
                                  flexDirection="row", alignItems="center",
                                  gap=18, width="100%", height="auto", padding=0)
            badge = nb.container(row_id, children=[],
                                 background="#212226", width="94px", height="44px",
                                 borderRadius=999, alignItems="center",
                                 justifyContent="center", flexShrink="0", padding=0)
            badge_txt = nb.title(badge, f"{n:02d}",
                                 fontSize=22, color="#F7F3EE", textAlign="center", fontWeight="bold")
            nb.add_children(badge, [badge_txt])
            nb.add_children(row_id, [badge])
            item_txt = nb.text(row_id, item, fontSize=20, color=tc_muted, textAlign="left", fontWeight="500")
            nb.add_children(row_id, [item_txt])
            nb.add_children(col, [row_id])
        nb.add_children(grid, [col])

    children.append(grid)

    return nb.build(children, {
        "background": bg, "alignItems": "flex-start",
        "justifyContent": "flex-start", "gap": 0, "padding": "62px 56px 54px 56px",
    })
...
def build_content(slide: dict) -> str:
    ...
    tid = nb.title("ROOT", c.get("title", "Section"),
                   fontSize=34, color=tc, textAlign="left", fontWeight="bold")
    children.append(tid)
    ...
    if c.get("body"):
        bid = nb.text("ROOT", c["body"], fontSize=18, color=tc_muted, textAlign="left")
        children.append(bid)

    for bullet in c.get("bullets", []):
        row_id = nb.container("ROOT", children=[],
                              flexDirection="row", alignItems="flex-start",
                              gap=12, padding="8px 0", height="auto")
        dot_id = nb.container(row_id, children=[],
                              background=accent, width="32px", height="2px",
                              borderRadius=3, margin="12px 0 0 0", flexShrink="0")
        nb.add_children(row_id, [dot_id])
        btxt = nb.text(row_id, bullet, fontSize=16, color=tc_muted, textAlign="left")
...
def build_two_columns(slide: dict) -> str:
    ...
    def build_col(col_data: dict, parent_id: str) -> str:
        ...
        for item in col_data.get("items", []):
            ...
            dot = nb.container(irow, children=[],
                               background=accent, width="24px", height="2px",
                               borderRadius=3, margin="10px 0 0 0", flexShrink="0")
            nb.add_children(irow, [dot])
            itxt = nb.text(irow, item, fontSize=15, color=tc_muted, textAlign="left")
    ...
    if light_bg:
        nb.nodes[right_id]["props"]["borderLeft"] = f"2px solid {accent}"
        nb.nodes[right_id]["props"]["padding"] = "8px 0 8px 20px"
...
def build_big_number(slide: dict) -> str:
    ...
    num_id = nb.title("ROOT", c.get("number", "0"),
                      fontSize=110, color=tc, textAlign="center", fontWeight="bold")
    children.append(num_id)

    label_id = nb.title("ROOT", c.get("label", ""),
                        fontSize=26, color=tc, textAlign="center", fontWeight="600")
    children.append(label_id)

    if c.get("description"):
        bar = nb.container("ROOT", children=[],
                           background="rgba(255,255,255,0.55)", height="2px",
                           width="64px", borderRadius=2, alignSelf="center", margin="8px 0")
        children.append(bar)
        desc_id = nb.text("ROOT", c["description"],
                          fontSize=16, color=tc_muted, textAlign="center")
...
def build_conclusion(slide: dict) -> str:
    ...
    tid = nb.title("ROOT", c.get("title", "Thank You!"),
                   fontSize=54, color=tc, textAlign="center", fontWeight="bold")
    children.append(tid)

    bar = nb.container("ROOT", children=[],
                       background=slide.get("accent", "#B3262D") if is_light_color(bg) else "rgba(255,255,255,0.5)", height="2px",
                       width="72%", borderRadius=2,
