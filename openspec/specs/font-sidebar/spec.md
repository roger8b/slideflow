...
The system SHALL replace the native `<select>` font control in `ContextualToolbar` with a clickable button that displays the current font name. Clicking this button SHALL open the `FontSidebar` panel sliding in from the **left** side of the editor.
...
- **THEN** the `FontSidebar` slides in from the left, the font button appears active, and the `ColorSidebar` (if open) is closed
