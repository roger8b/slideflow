...
| **RF 004** | **Geração de Slide Único (Contexto de Edição)** | **Seção 3.3 (ADK)** | O `AILayoutGenerator` dentro do editor de slide deve chamar uma procedure `generateSlide` dedicada que executa **apenas** o loop Designer → QA Reviewer (sem Starter, sem Writer). Aceita `{ prompt, context? }` e retorna um único `craftJson` via SSE. Evita custo desnecessário de pipeline completo para substituir o layout de um único slide. |
| **RF 005** | **Repositório de Storytellings** | **Seção 3.1 + Frontend** | O usuário deve poder gerar e salvar *storytellings* (estruturas macro de slides) de forma independente da geração visual. A aba "Storytellings" na sidebar esquerda oferece: (1) textarea com sugestões de templates estruturados (Pitch, Relatório, Kickoff, etc.) que pré-preenchem o campo com scaffolding; (2) chama `generateStorytelling` (apenas Starter_Agent via SSE); (3) renderiza a lista editável de `MacroNodes` para aprovação ou regeneração; (4) lista de storytellings salvos em `localStorage`. |
| **RF 006** | **Geração de Deck Completo a partir de Storytelling** | **Seção 3.3 (ADK)** | A partir de um storytelling aprovado, o usuário clica em "Gerar Deck". O frontend chama `generateDeckFromStorytelling` que recebe `{ macroNodes }`, pula o Starter_Agent e executa Writer_Agent (com RAG do Brand Kit) + SlideLoopAgent × N slides. A cada evento `slide_complete` via SSE, um novo nó é adicionado ao canvas ReactFlow em tempo real. Ao `complete`, o canvas exibe o deck finalizado via `fitView()`. |
